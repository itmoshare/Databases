...
public class FullTextIndex
{
    ...
    @Procedure(value = "example.getRouteWorkingMinutes", mode=Mode.READ)
    @Description("return minutes diff between route start and route finish")
    public Stream<WorkMinutes> getRouteWorkingMinutes( @Name("routeId") String routeId) {
        try {
            ...
            for (Relationship rel: rels) {
                String timeValue = (String) rel.getEndNode().getProperty("time");
            ...
            if(max.compareTo(min) >=1)
                result = (int)Math.abs(ChronoUnit.MINUTES.between(min,max));
            else
                result = (int)(1440-Math.abs(ChronoUnit.MINUTES.between(min,max)));
            WorkMinutes wm = new WorkMinutes(result);
            WorkMinutes[] wma = new WorkMinutes[1];
            wma[0] = wm;
            return Arrays.stream(wma);
        } catch (Exception e) {
            return Arrays.stream(new WorkMinutes[0]);
        }
    }
    /**
    ...
    public static class WorkMinutes {
        public Double workMinutes;
        public WorkMinutes(int workMinutes) { this.workMinutes = (double)workMinutes;}
    }
